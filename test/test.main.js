...
tape( 'the function has an arity of 3', function test( t ) {
	t.strictEqual( gindexOfTruthy.length, 3, 'has expected arity' );
	t.end();
});
...
tape( 'the function returns the index of the first truthy element', function test( t ) {
	...
	// Nonnegative stride...
	actual = gindexOfTruthy( x.length, x, 1 );
	t.strictEqual( actual, 1, 'returns expected value' );

	actual = gindexOfTruthy( 3, x, 2 );
...
tape( 'the function returns `-1` if provided an `N` parameter which is less than or equal to zero', function test( t ) {
	...
	actual = gindexOfTruthy( 0, [ 1.0, 2.0, 3.0 ], 1 );
	t.strictEqual( actual, -1, 'returns expected value' );

	actual = gindexOfTruthy( -1, [ 1.0, 2.0, 3.0 ], 1 );
	t.strictEqual( actual, -1, 'returns expected value' );
...
tape( 'the function returns `-1` if provided an `N` parameter which is less than or equal to zero (accessors)', function test( t ) {
	...
	actual = gindexOfTruthy( 0, toAccessorArray( [ 1.0, 2.0, 3.0 ] ), 1 );
	t.strictEqual( actual, -1, 'returns expected value' );

	actual = gindexOfTruthy( -1, toAccessorArray( [ 1.0, 2.0, 3.0 ] ), 1 );
	t.strictEqual( actual, -1, 'returns expected value' );
